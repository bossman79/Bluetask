...
{
    public class NetworkHistoryToPointsConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, string language)
        {
            try
            {
                ...
                // Use larger dimensions for the main Network chart
                double width = 600; // Adjusted for larger chart area
                double height = 300; // Adjusted for larger chart area
                
                // For network, we need to auto-scale based on data
                double maxValue = history.Count > 0 ? history.Max() * 1.1 : 1.0;
            ...
            {
                System.Diagnostics.Debug.WriteLine($"NetworkHistoryToPointsConverter error: {ex.Message}");
                return new PointCollection();
            }
        }
