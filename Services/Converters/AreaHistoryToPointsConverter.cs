...
{
    // Produces a closed polygon for an area-under-curve chart
    public class AreaHistoryToPointsConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, string language)
        {
            try
            {
                ...
                width -= padding * 2;
                height -= padding * 2;
                if (history.Count < 2 || width <= 0 || height <= 0) return new PointCollection();

                double maxValue = 100.0; // percent
