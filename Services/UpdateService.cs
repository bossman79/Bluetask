...
namespace Bluetask.Services
{
	public sealed class UpdateService
	{
		...
		private UpdateService()
		{
			...
			try
			{
				var settingsToken = SettingsService.UpdateAuthToken;
				if (!string.IsNullOrWhiteSpace(settingsToken))
				{
					SetAuthToken(settingsToken);
				}
				SettingsService.UpdateSettingsChanged += () =>
				{
					try { SetAuthToken(SettingsService.UpdateAuthToken); } catch { }
					try { Configure(SettingsService.UpdateRepoOwner, SettingsService.UpdateRepoName, SettingsService.UpdateIncludePrereleases); } catch { }
				};
			}
			catch { }
		}
		...
		public async Task CheckForUpdatesAsync(CancellationToken cancellationToken = default)
		{
			...
			try
			{
				// Robust check: compare latest commit touching Program/ vs newest local Program file timestamp
				var programHead = await FetchLatestProgramCommitAsync(cancellationToken).ConfigureAwait(false);
				var remoteSha = programHead.sha;
				var remoteDateUtc = programHead.date.UtcDateTime;
				_lastInfo = new UpdateInfo { TagName = remoteSha, ReleaseName = remoteSha };
				var localNewestUtc = GetLocalProgramNewestWriteTimeUtc();
				bool decided = false;
				if (!string.IsNullOrEmpty(remoteSha) && remoteDateUtc > DateTime.MinValue.AddYears(1))
				{
					// Allow small clock drift tolerance
					var tolerance = TimeSpan.FromMinutes(1);
					var shouldUpdate = localNewestUtc == DateTime.MinValue || (remoteDateUtc - localNewestUtc) > tolerance;
					if (shouldUpdate)
					{
				...
				if (!decided)
				{
					// Fallback to previous repo head comparison if Program-specific check failed
		...
		private async Task<(string sha, DateTimeOffset date, string branch)> FetchLatestProgramCommitAsync(CancellationToken ct)
		{
			try
			{
				...
				using (var repoResp = await _httpClient.GetAsync(repoUri, ct).ConfigureAwait(false))
				{
					...
					var commitsUri = $"https://api.github.com/repos/{_repoOwner}/{_repoName}/commits?sha={Uri.EscapeDataString(defaultBranch)}&path=Program&per_page=1";
					using var commitsResp = await _httpClient.GetAsync(commitsUri, ct).ConfigureAwait(false);
					commitsResp.EnsureSuccessStatusCode();
					var commitsStream = await commitsResp.Content.ReadAsStreamAsync(ct).ConfigureAwait(false);
					using var commitsDoc = await JsonDocument.ParseAsync(commitsStream, cancellationToken: ct).ConfigureAwait(false);
					if (commitsDoc.RootElement.ValueKind != JsonValueKind.Array || commitsDoc.RootElement.GetArrayLength() == 0)
					{
						return (string.Empty, DateTimeOffset.MinValue, defaultBranch);
					}
					var el = commitsDoc.RootElement[0];
					var sha = el.TryGetProperty("sha", out var shaEl) ? (shaEl.GetString() ?? string.Empty) : string.Empty;
					DateTimeOffset date = DateTimeOffset.MinValue;
					try
					{
						var commit = el.GetProperty("commit");
						if (commit.TryGetProperty("committer", out var committer) && committer.TryGetProperty("date", out var dateEl))
						{
							if (DateTimeOffset.TryParse(dateEl.GetString(), out var dto)) date = dto;
						}
						else if (commit.TryGetProperty("author", out var author) && author.TryGetProperty("date", out var aDate))
						{
							if (DateTimeOffset.TryParse(aDate.GetString(), out var dto2)) date = dto2;
						}
					}
					catch { }
					return (sha, date, defaultBranch);
				}
			}
			catch { return (string.Empty, DateTimeOffset.MinValue, ""); }
		}

		private DateTime GetLocalProgramNewestWriteTimeUtc()
		{
			try
			{
				DateTime newest = DateTime.MinValue;
				// 1) Check alongside the running executable (installed/unpackaged scenario)
				try
				{
					var exePath = Environment.ProcessPath ?? (Process.GetCurrentProcess().MainModule?.FileName ?? string.Empty);
					var baseDir = string.IsNullOrEmpty(exePath) ? AppContext.BaseDirectory : (Path.GetDirectoryName(exePath) ?? AppContext.BaseDirectory);
					var programDir = Path.Combine(baseDir, "Program");
					newest = MaxWriteTimeUtc(newest, programDir);
				}
				catch { }

				// 2) When running from source, also check the project's Program folder
				try
				{
					var root = LocateProjectRoot(AppContext.BaseDirectory);
					if (!string.IsNullOrEmpty(root))
					{
						var repoProgram = Path.Combine(root!, "Program");
						newest = MaxWriteTimeUtc(newest, repoProgram);
					}
				}
				catch { }
		...
		private static DateTime MaxWriteTimeUtc(DateTime current, string directory)
		{
			try
			{
				if (!Directory.Exists(directory)) return current;
				DateTime newest = current;
		...
		public bool ScheduleReplaceAndRestart(string stagingRoot)
		{
			try
			{
				...
				sb.AppendLine(":wait");
				sb.AppendLine("for /f \"tokens=2,3,4,5*\" %%a in ('tasklist /FI \"PID eq %PID%\" ^| findstr /R /C:\"^[^ ]\\+  \\+[0-9]\\+  \\+%PID%\\+\"') do set found=%%c");
