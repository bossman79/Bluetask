...
namespace Bluetask.Services
{
    public sealed class ProcessMonitorService
    {
        ...
        // Cache expensive ReadCategory() results to reduce CPU spikes
        private Dictionary<int, long> _cachedWorkingSet = new();
        private Dictionary<int, long> _cachedPrivateWs = new();
        private DateTime _lastMemoryCacheRefresh = DateTime.MinValue;
        private readonly TimeSpan _memoryCacheTtl = TimeSpan.FromSeconds(2); // Refresh every 2 seconds instead of every cycle
        ...
        public IReadOnlyList<ProcessInfo> SampleProcesses()
        {
            ...
            // Cache expensive ReadCategory() to reduce CPU spikes - refresh every 2s instead of every cycle
            Dictionary<int, long> workingSetByPid;
            Dictionary<int, long> privateWsByPid;
            
            lock (_memoryCacheLock)
            {
                if ((nowUtc - _lastMemoryCacheRefresh) >= _memoryCacheTtl || _cachedWorkingSet.Count == 0)
                {
                    // Time to refresh cache
                    var newWorkingSet = new Dictionary<int, long>();
                    var newPrivateWs = new Dictionary<int, long>();
                    try
                    {
                        ...
                        if (data != null && data.Contains("ID Process"))
                        {
                            ...
                            foreach (System.Collections.DictionaryEntry entry in idColl)
                            {
                                try
                                {
                                    ...
                                    {
                                        try { newWorkingSet[pid] = (long)wsColl[instanceName].RawValue; } catch { }
                                    }
                                    if (pwsColl != null && pwsColl.Contains(instanceName))
                                    {
                                        try { newPrivateWs[pid] = (long)pwsColl[instanceName].RawValue; } catch { }
                                    }
                                }
                                catch { }
                            }
                    ...
                    _cachedWorkingSet = newWorkingSet;
                    _cachedPrivateWs = newPrivateWs;
                    _lastMemoryCacheRefresh = nowUtc;
                }
                
                // Use cached values
                workingSetByPid = _cachedWorkingSet;
                privateWsByPid = _cachedPrivateWs;
            }
            ...
            // Normalize names and use partial sort for better performance
            // Only sort the top processes we care about instead of full sort
            ...
            // Optimized: Use partial sort (nth element) when we only need top N
            // This is O(n) instead of O(n log n) for full sort
            if (list.Count <= 200)
            {
                // All processes fit, just return sorted
                return list.OrderByDescending(pi => pi.CpuPercent).ToArray();
            }
            else
            {
                // Use partial sort - only sort top 200
                return list
        ...
        // Cleanup method to dispose performance counters and prevent resource leaks
        public void Shutdown()
        {
            try
            {
                lock (_gpuCountersLock)
                {
                    foreach (var kvp in _gpuCountersByInstance)
                    {
                        try { kvp.Value?.Dispose(); } catch { }
                    }
                    _gpuCountersByInstance.Clear();
                    _gpuPidByInstance.Clear();
                }
            }
            catch { }
        }
