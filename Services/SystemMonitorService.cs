...
namespace Bluetask.Services
{
    public class SystemMonitorService
    {
        ...
        // Trigger async initialization early - call from App startup
        public void WarmupAsync()
        {
            // Kick off hardware initialization in background immediately
            EnsureComputerOpened();
            
            // Also pre-warm network interfaces on background thread
            _ = System.Threading.Tasks.Task.Run(() =>
            {
                try { EnsureNetworkInterfaces(); } catch { }
            });
        }

        private readonly Computer _computer;
        private bool _computerOpened;
        private bool _computerOpeningInProgress;
        private readonly object _updateLock = new object();
        private System.Threading.Tasks.Task? _computerOpenTask;
        private bool _disableProcPerfCounters;
        private bool _disableDiskIdleCounters;
        private NetworkInterface[] _networkInterfaces;
        private bool _networkInterfacesInitializing;
        ...
        private double _lastCpuPowerWatts;

        // Async drive metadata cache to avoid blocking WMI queries
        private readonly Dictionary<string, (string brand, string mediaKind)> _driveMetadataCache = new();
        private readonly System.Threading.SemaphoreSlim _driveMetadataSemaphore = new System.Threading.SemaphoreSlim(1, 1);
        private readonly HashSet<string> _driveMetadataInProgress = new();
        ...
        private void EnsureComputerOpened()
        {
            ...
            if (!_computerOpeningInProgress && _computerOpenTask == null)
            {
                lock (_updateLock)
                {
                    if (!_computerOpeningInProgress && _computerOpenTask == null)
                    {
                        _computerOpeningInProgress = true;
                        _computerOpenTask = System.Threading.Tasks.Task.Run(() =>
                        {
                            try 
                            { 
                                _computer.Open(); 
                            ...
                            {
                                _computerOpeningInProgress = false;
                            }
                        });
                    }
                }
            ...
            // Non-blocking: if computer is still opening, return immediately
            // Next Update() call will use the opened hardware when ready
        }
        ...
        private void EnsureNetworkInterfaces()
        {
            try
            {
                ...
                {
                    if (_networkInterfacesInitializing) return; // Already initializing, skip
                    
                    _networkInterfacesInitializing = true;
                    
                    // Run network interface enumeration on background thread - it can be slow on first call
                    _ = System.Threading.Tasks.Task.Run(() =>
                    {
                        try
                        {
                        ...
                        {
                            _networkInterfacesInitializing = false;
                        }
                    });
                }
            }
            catch { _networkInterfaces = Array.Empty<NetworkInterface>(); _networkInterfacesInitializing = false; }
        }
        ...
        private void EnsureDiskCounters()
        {
            ...
            try
            {
                foreach (var d in System.IO.DriveInfo.GetDrives().Where(d => d.IsReady && d.DriveType == System.IO.DriveType.Fixed))
                {
                    ...
                    // Don't fall back to PhysicalDisk - it causes multiple logical drives 
                    // on the same physical disk to show identical activity
                    // Instead, keep counter null and show 0 activity for drives without LogicalDisk counters
                    // This is more accurate than showing misleading duplicate data
        ...
        public void Update()
        {
            ...
            lock (_updateLock)
            {
                ...
                // Skip expensive hardware updates if not yet ready - critical for fast startup
                if (!_computerOpened)
                {
                    return;
                }
                ...
                if (!_disableDiskIdleCounters)
                {
                    foreach (var kvp in _idleCountersByDrive.ToArray())
                    {
                        try
                        {
                            ...
                            // Light smoothing to reduce counter jitter (less aggressive for more distinct per-drive readings)
                            if (_lastDriveActivityByLetter.TryGetValue(kvp.Key, out var prev))
                            {
                                raw = (prev * 0.3) + (raw * 0.7); // 30% previous, 70% current (was 60/40)
                            }
        ...
        {
            // Return placeholder if hardware not ready - critical for fast startup
            if (!_computerOpened)
            {
                return new CpuInfo { Name = "CPU", Usage = 0, CurrentClockGhz = 0, UsageHistory = new System.Collections.Generic.List<double>() };
            }
            ...
            // Reuse last good CPU power sample to avoid transient 0.0 W readings from sensors
            try
            {
                if (powerWatts <= 0.0 && _lastCpuPowerWatts > 0.0)
                {
                    powerWatts = _lastCpuPowerWatts;
                }
                else if (powerWatts > 0.0)
                {
                    _lastCpuPowerWatts = powerWatts;
                }
            }
            catch { }
        ...
        {
            // Return empty if hardware not ready - critical for fast startup
            if (!_computerOpened) 
            {
                yield break;
            }
        ...
        public RamInfo GetRamInfo()
        {
            ...
            if (!_computerOpened)
            {
                return new RamInfo { Usage = 0, UsedAndTotal = "Loading...", Type = "RAM" };
            }
        ...
        public IEnumerable<Bluetask.Models.StorageInfo> GetDriveInfo()
        {
            ...
            for (int i = 0; i < realCount; i++)
            {
                ...
                // Get cached metadata or return placeholder - trigger async load if not cached
                string brand = string.Empty;
                string mediaKind = string.Empty;
                if (_driveMetadataCache.TryGetValue(letter, out var cached))
                {
                    brand = cached.brand;
                ...
                {
                    // Trigger async background load without blocking
                    _ = LoadDriveMetadataAsync(letter);
                }
        ...
        // Async method to load drive metadata (brand and media kind) without blocking
        private async System.Threading.Tasks.Task LoadDriveMetadataAsync(string letter)
        {
            // Prevent duplicate concurrent requests for same drive
            bool shouldLoad = false;
            await _driveMetadataSemaphore.WaitAsync().ConfigureAwait(false);
            try
            {
                if (!_driveMetadataCache.ContainsKey(letter) && !_driveMetadataInProgress.Contains(letter))
                {
                    _driveMetadataInProgress.Add(letter);
                    shouldLoad = true;
                }
            }
            finally
            {
                _driveMetadataSemaphore.Release();
            }
            ...
            try
            {
                // Run expensive WMI queries on background thread
                var result = await System.Threading.Tasks.Task.Run(() =>
                {
                    string brand = GetDriveBrandForLetter(letter);
                    string mediaKind = GetDriveMediaKind(letter);
                    return (brand, mediaKind);
                }).ConfigureAwait(false);

                await _driveMetadataSemaphore.WaitAsync().ConfigureAwait(false);
                try
                {
                    _driveMetadataCache[letter] = result;
                    _driveMetadataInProgress.Remove(letter);
                }
                finally
                {
                    _driveMetadataSemaphore.Release();
            ...
            catch
            {
                ...
                {
                    _driveMetadataInProgress.Remove(letter);
                    // Cache empty values to avoid repeated failed attempts
                    _driveMetadataCache[letter] = (string.Empty, string.Empty);
                }
                finally
                {
                    _driveMetadataSemaphore.Release();
        ...
        {
            // Ensure NIC list and initial counters are populated before sampling
            EnsureNetworkInterfaces();
            
            // Return placeholder if network interfaces not yet initialized - critical for fast startup
            if (_networkInterfaces == null || _networkInterfaces.Length == 0)
            {
        ...
        public void Shutdown()
        {
            try
            {
                ...
                try
                {
                    lock (_updateLock)
                    {
                        foreach (var kvp in _idleCountersByDrive)
                        {
                ...
                // Close LibreHardwareMonitor Computer to release driver handles
                try
                {
                    if (_computerOpened && _computer != null)
                    {
                        lock (_updateLock)
                        {
        ...
        private void EnsureEtwNetworkAccounting()
        {
            ...
            lock (_etwInitLock)
            {
                ...
                try
                {
                    // Only enable ETW when user opts into advanced monitoring
                    if (!SettingsService.AdvancedMonitoring)
                    {
                        _etwActive = false;
                        return;
                    }
                    ...
                    {
                        // Do not attach to the global kernel logger; simply disable ETW if private session fails
                        _etwActive = false;
                    }
                }
                catch
