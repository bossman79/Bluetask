...
namespace Bluetask.Services
{
	...
	public static class SettingsService
	{
		...
		// Advanced monitoring (enables ETW kernel session usage)
		private static bool _advancedMonitoring = false;
		...
		private static void LoadSettings()
		{
			...
			_advancedMonitoring = SettingsServiceHelper.GetValue(nameof(AdvancedMonitoring), false);
		...
		public static bool AdvancedMonitoring
		{
			get { lock (_lock) { return _advancedMonitoring; } }
			set
			{
				bool changed;
				lock (_lock)
				{
					if (_advancedMonitoring == value) return;
					_advancedMonitoring = value;
					changed = true;
				}
				if (changed)
				{
					SettingsServiceHelper.SetValue(nameof(AdvancedMonitoring), value);
				}
			}
		}
