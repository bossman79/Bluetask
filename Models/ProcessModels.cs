...
namespace Bluetask.Models
{
    public sealed class ProcessModel : ObservableObject
    {
        ...
        public double CpuPercent
        {
            ...
            set
            {
                // Only notify formatted display if the displayed text actually changes at 0.1% precision
                var oldDisplay = CpuPercentDisplay; // based on previous _cpuPercent
                if (SetProperty(ref _cpuPercent, value))
                {
                    var newDisplay = ($"{_cpuPercent:F1}%");
                    if (!string.Equals(oldDisplay, newDisplay, System.StringComparison.Ordinal))
                    {
        ...
        public long MemoryBytes
        {
            ...
            set
            {
                var oldDisplay = MemoryDisplay;
                if (SetProperty(ref _memoryBytes, value))
                {
                    var newDisplay = FormatBytes(_memoryBytes);
                    if (!string.Equals(oldDisplay, newDisplay, System.StringComparison.Ordinal))
                    {
        ...
        public double GpuPercent
        {
            ...
            set
            {
                var oldDisplay = GpuPercentDisplay;
                var oldDisplayOrDash = GpuPercentDisplayOrDash;
                if (SetProperty(ref _gpuPercent, value))
                {
                    var newDisplay = ($"{_gpuPercent:F1}%");
                    var newDisplayOrDash = _gpuPercent <= 0.0 ? "-" : newDisplay;
                    if (!string.Equals(oldDisplay, newDisplay, System.StringComparison.Ordinal))
                    {
                        OnPropertyChanged(nameof(GpuPercentDisplay));
                    }
                    if (!string.Equals(oldDisplayOrDash, newDisplayOrDash, System.StringComparison.Ordinal))
                    {
                        OnPropertyChanged(nameof(GpuPercentDisplayOrDash));
                    }
                }
            }
        }
