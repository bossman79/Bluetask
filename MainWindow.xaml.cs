...
namespace Bluetask
{
    ...
    public sealed partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            // Start hardware monitoring initialization in background immediately
            // This allows expensive LibreHardwareMonitor setup to happen while UI renders
            System.Threading.Tasks.Task.Run(() =>
            {
                try { Services.SystemMonitorService.Shared.WarmupAsync(); } catch { }
            });
        ...
        private void MainWindow_Activated(object sender, WindowActivatedEventArgs args)
        {
            ...
            // Navigate without transition animation for faster initial load
            var transitionInfo = new Microsoft.UI.Xaml.Media.Animation.SuppressNavigationTransitionInfo();
            try { ContentFrame.Navigate(typeof(Bluetask.Views.DashboardPage), null, transitionInfo); } catch { }
        ...
        {
            // Use NavigationTransitionInfo to improve perceived navigation speed
            var transitionInfo = new Microsoft.UI.Xaml.Media.Animation.SuppressNavigationTransitionInfo();
            
            switch ((e.NavigationTarget ?? string.Empty))
            {
                case "Dashboard":
                    ContentFrame.Navigate(typeof(Bluetask.Views.DashboardPage), null, transitionInfo);
                    break;
                case "Processes":
                    ContentFrame.Navigate(typeof(Bluetask.Views.ProcessesPage), null, transitionInfo);
                    break;
                case "Performance":
                    ContentFrame.Navigate(typeof(Bluetask.Views.PerformancePage), null, transitionInfo);
                    break;
                case "Stability Center":
                    ContentFrame.Navigate(typeof(Bluetask.Views.StabilityCenterPage), null, transitionInfo);
                    break;
                case "Settings":
                    ContentFrame.Navigate(typeof(Bluetask.Views.SettingsPage), null, transitionInfo);
                    break;
            }
        }
    }
