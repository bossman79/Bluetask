...
namespace Bluetask.ViewModels
{
    ...
    public sealed partial class SidebarItem : ObservableObject
    {
        ...
        // Convenience flags for XAML templates to toggle custom icons
        public bool IsMemoryType => Type == PerformanceItemType.Memory;
        public bool IsGpuType => Type == PerformanceItemType.Gpu;
        public bool HasCustomIcon => IsMemoryType || IsGpuType;
        public bool HasDefaultGlyph => !HasCustomIcon;
    }
    ...
    public sealed partial class PerformanceViewModel : ObservableObject, IDisposable
    {
        ...
        public PerformanceViewModel(bool deferStart = false)
        {
            _systemMonitor = SystemMonitorService.Shared;
            _processMonitor = new ProcessMonitorService();
            _dispatcher = DispatcherQueue.GetForCurrentThread();

            // Defer expensive WMI boot time query to background to speed up initialization
            _systemStartTime = DateTime.Now.AddDays(-1); // placeholder
        ...
        private async Task UpdateLoopAsync()
        {
            ...
            while (!_cts.IsCancellationRequested)
            {
                try
                {
                    ...
                    // Defer expensive operations: skip first 2 cycles entirely, then update every 3rd cycle
                    var shouldUpdateExpensive = (_updateCycleCount > 2) && ((updateCounter % 3) == 0);
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        try { await Task.Delay(Math.Min(5000, _interval.Milliseconds * 2), _cts.Token).ConfigureAwait(false); } 
                        catch (System.OperationCanceledException) { break; } // Catches both OperationCanceledException and TaskCanceledException
                        catch { }
                    }
                }
                                  ...
                                  bool expensive, int procCount, int threadCount, int handleCount, string cpuSpeed, string uptime)
        {
            ...
            try
            {
                ...
                // Derive instantaneous rates from the most recent history sample so UI numbers match the chart
                double upMbps = (target.UploadHistory != null && target.UploadHistory.Count > 0) ? target.UploadHistory[target.UploadHistory.Count - 1] : 0.0;
                double downMbps = (target.DownloadHistory != null && target.DownloadHistory.Count > 0) ? target.DownloadHistory[target.DownloadHistory.Count - 1] : 0.0;
                target.UploadSpeed = FormatAxisRate(upMbps);
                target.DownloadSpeed = FormatAxisRate(downMbps);
            ...
            // Keep sidebar value in sync with display/chart by deriving from latest samples
            try
            {
                double upMbps = (Network?.UploadHistory != null && Network.UploadHistory.Count > 0) ? Network.UploadHistory[Network.UploadHistory.Count - 1] : 0.0;
                double downMbps = (Network?.DownloadHistory != null && Network.DownloadHistory.Count > 0) ? Network.DownloadHistory[Network.DownloadHistory.Count - 1] : 0.0;
                NetworkSpeed = FormatAxisRate(upMbps + downMbps);
            }
            catch { NetworkSpeed = FormatNetworkSpeed(net); }
